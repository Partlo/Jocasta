...
class ObjectionResult:
    def __init__(self, nominator: str, objector: str, addressed: bool, overdue: bool, last_date: datetime, lines):
...
def is_review_note(s):
    return "(review note)" in s.lower().replace("reviewing", "review") or \
           "review note:" in s.lower().replace("reviewing", "review")
...
def build_objection_trees(lines) -> List[List[Tuple[bool, dict]]]:
        ...
        try:
            if objections_found:
                ...
                if len(bullets) == 1 and line.startswith(":*") and 1 in current_tree:
                    ...
                    print(f"Unexpected state: {len(bullets)}, {line}")
            ...
            print("X", type(e), e, line)
    return sections


def extract_actual_objections(section: List[Tuple[bool, dict]]) -> Tuple[str, List[ObjectionTree]]:
    current_user = None
    tree_dates = {}
    results = []
    user_ordering = {}
    for nested, tree in reversed(section):
        if not tree:
            continue
...
def identify_overdue_objections(nominator: str, user: str, trees: List[ObjectionTree]) -> List[ObjectionResult]:
    overdue = []
    now = datetime.now()
    for tree_data in trees:
        ...
        if target.date is None:
            print(f"Cannot check date for objections from {user}")
            continue
        ...
        duration = now - date
        if duration.days >= 6 and len(counts) % 2 == 0:
            overdue.append(ObjectionResult(nominator=nominator, objector=tree_data.user, addressed=True,
                                           overdue=duration.days >= 7, last_date=target.date, lines=tree_data.lines))
        elif duration.days >= 6:
            overdue.append(ObjectionResult(nominator=nominator, objector=tree_data.user, addressed=False,
                                           overdue=duration.days >= 7, last_date=target.date, lines=tree_data.lines))
        elif max(counts) % 2 == (1 if tree_data.nested else 0):
            print(f"{date} is within time window, skipping response from nominator - {len(counts) % 2 == 0}")
        else:
            print(f"{date} is within time window, skipping objection from {user} - {len(counts) % 2 == 1}")
    return overdue


def examine_objections_on_nomination(page: Page):
    """ :rtype: tuple[str, dict[bool, dict[str, dict[datetime, list[ObjectionResult]]]]] """
    try:
        text = page.get()
        nominator = find_nominator(text)
        sections = build_objection_trees(text.splitlines())

        result_map = {True: {}, False: {}}
        for section in sections:
            user, results = extract_actual_objections(section)
            objections = identify_overdue_objections(nominator, user, results)
...
def examine_nomination_and_prepare_results(page: Page):
    """ :rtype: tuple[list[str], list[tuple[str, str]]] """
    nominator, objection_data = examine_objections_on_nomination(page)
    if not nominator:
        return [], []
    ...
    for is_addressed, a_data in objection_data.items():
        ...
        for user, user_data in a_data.items():
            ...
            if oc:
                overdue.append(f"{oc} objections from {user} have been {a_str} for 7 or more days")
            if nc and is_addressed:
                normal.append((user, f"{nc} of your objections have been {a_str} for 6 days"))
            elif nc:
                normal.append((nominator, f"{nc} objections from {user} have been {a_str} for 6 days"))

    return overdue, normal


def check_active_nominations_on_page(site, nom_type, page_name):
    page = Page(site, NOM_TYPES[nom_type].nomination_page + "/" + page_name)
    if not page.exists():
        raise Exception(f"{nom_type} {page_name} does not exist")
    o, n = examine_nomination_and_prepare_results(page)
    return {page.full_url(): o}, {page.full_url(): n}


def check_active_nominations(site, nom_type: str):
    """:rtype: tuple[dict[str, list[str]], dict[str, list[tuple[str, str]]]]"""
    category = Category(site, NOM_TYPES[nom_type].nomination_category)
    total_overdue, total_normal = {}, {}
    for nom in category.articles():
        if "/" not in nom.title():
            continue
        overdue, normal = examine_nomination_and_prepare_results(nom)
        if overdue:
